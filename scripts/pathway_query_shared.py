...
@dataclass(slots=True)
class PrimaryCompoundResolution:
    """One batch-analysis metabolite resolved to a single primary compound."""

    query: str
    status: str  # resolved | ambiguous | unmapped
    compound_id: str = ""
    chebi_accession: str = ""
    chebi_name: str = ""
    match_type: str = ""
...
@dataclass(slots=True)
class RecoveredCandidateAttempt:
    """One ranked strict ChEBI candidate plus its recovered query result."""

    candidate: ChEBIExactCandidate
    exact_main_match: bool
    main_name_similarity: float
    mapping_score: float
    result: QueryResult
...
def _candidate_primary_name(candidate: ChEBIExactCandidate) -> str:
    return (candidate.ascii_name or candidate.name or "").strip()


def _candidate_query_variants(text: str) -> dict[str, str]:
    return build_variants(text)


def _candidate_exact_main_match(candidate: ChEBIExactCandidate, query_variants: dict[str, str]) -> bool:
    primary_name = _candidate_primary_name(candidate)
    primary_variants = _candidate_query_variants(primary_name)
    return any(
        primary_variants.get(variant, "") and primary_variants.get(variant, "") == query_variants.get(variant, "")
        for variant in CHEBI_STRICT_VARIANT_ORDER
    )


def _candidate_main_similarity(candidate: ChEBIExactCandidate, query_variants: dict[str, str]) -> float:
    primary_name = _candidate_primary_name(candidate)
    primary_exact = _candidate_query_variants(primary_name).get("exact", "")
    query_exact = query_variants.get("exact", "")
    if not primary_exact or not query_exact:
        return 0.0
    return SequenceMatcher(None, primary_exact, query_exact).ratio()


def _result_mapping_score(result: QueryResult) -> float:
    if result.match_type == "exact_structure":
        return 1.0
    if result.projection_type == "structural_neighbor" or result.match_type == "structural_neighbor":
        return result.neighbor_similarity
    if result.mappings:
        return max(mapping.match_score for mapping in result.mappings)
    return 0.0


def _direct_recovered_chebi_result(
    query: str,
    candidate: ChEBIExactCandidate,
    state: dict[str, Any],
    top_k: int,
) -> QueryResult | None:
    base_mappings = _dedupe_mappings(state["compound_index"].get(candidate.compound_id, []))
    if not base_mappings:
        return None
    result = build_query_result(
        query=query,
        match_type="recovered_from_chebi",
        resolution_path="plain_text -> chebi_exact -> aracyc_mapping",
        neighbors=[NeighborHit(compound_id=candidate.compound_id, similarity=1.0, recovered_chebi_id=candidate.chebi_accession)],
        state=state,
        top_k=top_k,
        base_mappings=base_mappings,
        expanded_compound_ids=[candidate.compound_id],
    )
    if not result.recovered_chebi_id:
        result.recovered_chebi_id = candidate.chebi_accession
    return result


def _project_single_recovered_chebi_candidate(
    query: str,
    candidate: ChEBIExactCandidate,
    state: dict[str, Any],
    top_k: int,
    chebi_structures: dict[str, StructureRecord],
    arab_index: dict[str, Any],
) -> QueryResult | None:
...
def _rank_recovered_chebi_candidates(query: str, candidates: list[ChEBIExactCandidate]) -> list[tuple[ChEBIExactCandidate, bool, float]]:
    query_variants = build_variants(query)
    ranked: list[tuple[ChEBIExactCandidate, bool, float]] = []
    for candidate in candidates:
        exact_main_match = _candidate_exact_main_match(candidate, query_variants)
        main_name_similarity = _candidate_main_similarity(candidate, query_variants)
        ranked.append((candidate, exact_main_match, main_name_similarity))
    ranked.sort(
        key=lambda item: (
            1 if item[1] else 0,
            item[2],
            item[0].stars,
            -int(item[0].compound_id),
        ),
        reverse=True,
    )
    return ranked
...
def resolve_recovered_chebi_candidates(
    ...
    ranked_candidates = _rank_recovered_chebi_candidates(query, candidates)
    ranked_suggestions = tuple(
        f"{candidate.name} ({candidate.chebi_accession})"
        for candidate, _, _ in ranked_candidates[:MAX_FUZZY_SUGGESTIONS]
    )
    attempts: list[RecoveredCandidateAttempt] = []

    for candidate, exact_main_match, main_name_similarity in ranked_candidates:
        result = _direct_recovered_chebi_result(query, candidate, state, top_k=top_k)
        if result is None:
            result = _project_single_recovered_chebi_candidate(
                query,
                candidate,
                state,
                top_k,
        ...
        if result is None:
            continue
        attempts.append(
            RecoveredCandidateAttempt(
                candidate=candidate,
                exact_main_match=exact_main_match,
                main_name_similarity=main_name_similarity,
                mapping_score=_result_mapping_score(result),
                result=result,
            )
        )
    ...
    if attempts:
        attempts.sort(
            key=lambda attempt: (
                1 if attempt.exact_main_match else 0,
                attempt.main_name_similarity,
                attempt.mapping_score,
                attempt.candidate.stars,
                -int(attempt.candidate.compound_id),
            ),
            reverse=True,
        )
        ...
        if len(candidates) > 1:
            result.did_you_mean = ranked_suggestions
            result.note = (
                f"Recovered {len(candidates)} strict ChEBI candidates; selected "
                f"{best_attempt.candidate.chebi_accession} after ranking by primary-name priority "
                "and Arabidopsis mapping support."
            )
        return result
    ...
    return QueryResult(
        ...
        did_you_mean=ranked_suggestions,
        note=(
            f"Recovered {len(candidates)} strict ChEBI candidates and ranked them by primary-name priority, "
            "but none could be mapped back to Arabidopsis via direct mapping or high-confidence structure projection."
        ),
    )
...
def resolve_primary_compound(
    ...
    *,
    ...
    if result.resolution_path.endswith("ambiguous"):
        return PrimaryCompoundResolution(
            query=query,
            status="ambiguous",
            ...
            recovered_chebi_id=result.recovered_chebi_id,
        )

    if not result.mappings and not result.pathways:
        return PrimaryCompoundResolution(
            query=query,
            status="unmapped",
            match_type=result.match_type,
    ...
    top_pathway = result.pathways[0] if result.pathways else None
    top_compound_id = top_pathway.compound_id if top_pathway else result.mappings[0].compound_id
    top_mapping = next((mapping for mapping in result.mappings if mapping.compound_id == top_compound_id), None)
    chebi_name = top_pathway.chebi_name if top_pathway else (top_mapping.chebi_name if top_mapping else "")
    chebi_accession = top_mapping.chebi_accession if top_mapping else (f"CHEBI:{top_compound_id}" if top_compound_id else "")

    if result.match_type == "exact_structure":
        match_score = 1.0
    elif result.projection_type == "structural_neighbor" or result.match_type == "structural_neighbor":
        match_score = result.neighbor_similarity
    elif top_mapping is not None:
        match_score = top_mapping.match_score
    else:
        match_score = 0.0
